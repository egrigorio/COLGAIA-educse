    ...
    <div class="navbar bg-base-100">
        ...
        foreach($arr_items as $item) {
            if(isset($item['nome_curso'])) {
                if($query == '') {
                    echo '
                    <a href="' . $arrConfig['url_admin'] . 'curso.php?" class="btn btn-ghost text-sm bg-gray-200">' . $item['abreviatura'] . '</a>
                    ';
                } else {
                    echo '
            ...
            if($query == 'id_turma=' . $item['id']) {
                echo '
                <a href="' . $arrConfig['url_admin'] . 'turma.php?id_turma=' . $item['id'] . '" class="btn btn-ghost text-sm bg-gray-200">' . $item['nome_turma'] . '</a>
                ';
            } else {
                echo '
...
function professores_tabs_cursos() {
    ...
    <div class="w-full max-w-xs mx-auto pt-5">
        <h2 class=" text-lg mb-4 text-center">Adicionar professores ao curso</h2>

        <form method="post" action="' . $arrConfig['url_modules'] . 'trata_adicionar_user_curso.mod.php' . '" id="emailForm">
            <div class="flex mb-4 gap-2">
                ...
                <button type="button" id="add-email" class="btn text-xs py-1 px-2">
                    Adicionar
                </button>
...
<script>
    ...
    document.getElementById("add-email").onclick = function() {
        ...
        if(email) {
            ...
            newDiv.className = "flex items-center bg-white px-3 py-1 rounded shadow mb-1 text-xs";
