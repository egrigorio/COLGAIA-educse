...
if($tem_turma) {
    ...
    for($i = 1; $i < $duracao; $i++) {
        ...
        $res = my_query($sql);
        $sql = "SELECT * FROM rel_turma_user INNER JOIN users ON rel_turma_user.id_user = users.id WHERE users.cargo = 'aluno' AND rel_turma_user.id_turma = $id_turma_antiga";               
        $res = my_query($sql);                
        foreach($res as $rel) {
            $id_user = $rel['id_user'];            

            $sql = "INSERT INTO rel_turno_user ";
        ...
        $sql = "SELECT * FROM rel_turno_user";
